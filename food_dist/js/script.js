window.addEventListener("DOMContentLoaded", function () {
  ...
  next.addEventListener("click", function () {
    //сколько будет в offset (при каждом клике добавляется, ширина одного слайда )
    // или обнуляется offset, при достижении последнего слайда
    if (offset == deleteNotDigits(width) * (slides.length - 1)) {
      offset = 0;
    } else {
      offset += deleteNotDigits(width);
    }
    //передвигается карусель в зависимости от offset
